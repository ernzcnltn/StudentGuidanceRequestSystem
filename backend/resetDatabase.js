// backend/resetDatabase.js - FIXED VERSION
...
const pool = mysql.createPool({
  ...
  database: 'fiu_guidance_db', // Direct database connection
  waitForConnections: true,
  connectionLimit: 10,
  queueLimit: 0
});

// Create connection pool for system queries (without database)
...
async function resetDatabase() {
  console.log('🔄 Resetting FIU Guidance System Database...');
  
  try {
    // 1. Check if database exists using system connection
    console.log('🔍 Checking if database exists...');
    const [databases] = await systemPool.execute('SHOW DATABASES LIKE "fiu_guidance_db"');
    
    if (databases.length === 0) {
      console.log('❌ Database fiu_guidance_db does not exist!');
      console.log('💡 Creating database...');
      await systemPool.execute('CREATE DATABASE fiu_guidance_db');
      console.log('✅ Database created successfully');
    } else {
      console.log('✅ Database fiu_guidance_db exists');
    }

    // 2. Drop all tables in correct order (to handle foreign keys)
